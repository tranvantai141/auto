...
const ComplianceInfoSection = () => {
  ...
  const { complianceInformation } = response ?? { complianceInformation: [] };
  ...
  const subSectionForItem = (item: ComplianceItemModel) => {
    if (item.type === 'stateless_person' && item.data != null) {
      return (
        <SubSection>
          <GeneralInfoItem
  ...
  return (
    <ContentSection title={translate('compliance_info_title')}>
