...
export function useCustomerInforResult() {
  ...
  const searchCustomerInfo = useCallback(async (idNumber: string) => {
    ...
    try {
      ...
      if(!res || res === undefined) throw Error();
      ...
      if(!resGetMemoCif || resGetMemoCif === undefined) throw Error();
      ...
      if(!resGetAccountByCif || resGetAccountByCif === undefined) throw Error();

      // console.log('getAccounts : ' , resGetAccountByCif)
  ...
  const searchSupplymentalInfo = useCallback(async (transactionID: string) => {
    ...
    if (resGetSupplimentalInfo.code === 'SUCCESS')
    {
      supplementalInfoList = resGetSupplimentalInfo.supplementalInfoList ?? [];
      resultData = {
        supplementalInfo: {
  ...
  const searchEBankInfo = useCallback(async (transactionID: string) => {
    ...
    try {
      ...
      {
        eBankingList = resEbankingRes.customerInfo ?? [];
        resultData = {
          result: 'ETB',
          productResult: {
            ebanks: eBankingList ?? {},
