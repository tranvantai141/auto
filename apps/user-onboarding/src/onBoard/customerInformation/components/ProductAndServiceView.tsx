...
const ProductAndServiceContent = ({ resultData }: Props) => {
  ...
  const { fillerCard, errorDetail } = useProductionAndServices(false);
  const LineView = ({
    isHiddenBottomLine,
    color,
  }: {
  ...
  const RenderEBankingServicesView = ({
    ...
    const content = () => {
      if (errorDetail === 'ebank' || errorDetail === 'all') {
        return (
    ...
    if (errorDetail !== 'ebank' && errorDetail !== 'all' && resultData?.result === 'ETB') {
      if (
  ...
  const RenderNonPhysicalDebitCardView = ({
    ...
    const content = () => {
      if (errorDetail === 'card' || errorDetail === 'all') {
        return (
          <Text style={{ marginTop: 12, fontSize: 16, fontWeight: '400', color: Colors.red }}>
            {translate('get_info_fail_message')}
          </Text>
    ...
    if (errorDetail !== 'card' && errorDetail !== 'all' && resultData?.result === 'ETB') {
      if (
        (resultData.productResult.cards ?? []).length === 0 ||
  ...
  return (
    <View style={[styles.container]}>
      ...
      <LineView color={{ backgroundColor: Colors.green_70 }} />
